...
const createNewUser = async (req, res) => {
  ...
  let providedCompanyID = req.body.companyId;
  providedCompanyID = providedCompanyID.trim();
...
const viewUsers = async (req, res) => {
  try {
    let companyName = req.params.companyName;
    companyName = companyName.toLowerCase();
    const userQuery = `SELECT * FROM \`${companyName}_users\` order by department asc`;
    const [users] = await pool.query(userQuery);
    return res
      .status(200)
      .json({ message: "Users fetched successfully", result: users });
